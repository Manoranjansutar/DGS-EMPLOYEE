...
const EmployeeProvider = ({ children }) => {
  ...
  const loginEmployee = async (email, password) => {
    try {
      const response = await axios.post(
        "https://dgs-backend-yo9v.onrender.com/api/v1/employee/login",
        {
          email,
          password,
        }
  ...
  const getEmployee = async (id) => {
    try {
      await axios
        .get(
          `https://dgs-backend-yo9v.onrender.com/api/v1/employee/getSingleEmployee/${localStorage.getItem(
            "id"
          )}`
        )
        .then((res) => {
  ...
  const getNotification = async () => {
    ...
    try {
      const res = await axios.get(
        "https://dgs-backend-yo9v.onrender.com/api/v1/notification/getEmployeeNotifications"
      );
  ...
  const getLimitNotifications = async () => {
    ...
    try {
      const res = await axios.post(
        "https://dgs-backend-yo9v.onrender.com/api/v1/notification/getLimitEmployeeNotifications",
        { id }
      );
  ...
  const getUniqueRecipientsWithLatestMessage = async () => {
    ...
    try {
      await axios
        .post(
          "https://dgs-backend-yo9v.onrender.com/api/v1/messages/getUniqueRecipientsWithLatestMessageForEmployee",
          { sender, senderType }
        )
        .then((res) => {
          console.log(res);
