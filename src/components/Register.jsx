...
const schema = z.object({
  ...
  empId: z
    .string()
    .min(6, { message: "Employee Id is required must be min 6 digit" }),
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.post(
        "https://dgs-backend-yo9v.onrender.com/api/v1/employee/register",
        data
      );
      console.log(res);
      ...
      toast(
        <div className="w-full p-4 text-white bg-green-900 rounded-lg">
          <h1 className="text-md">Employee registered successfully</h1>
        </div>
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="lg:h-[70vh] md:h-[100%] mb-20 mt-12">
      ...
      <div className="flex flex-col items-center justify-center mt-10 ">
        ...
        <p className="text-gray-500">
          ...
          <Link to="/login">
            <span className="font-semibold text-gray-600">Log In </span>
          </Link>
        </p>
